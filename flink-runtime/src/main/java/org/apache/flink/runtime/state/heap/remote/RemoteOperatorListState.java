 ...
 */
class RemoteOperatorListState<K, N, V>
	extends AbstractRemoteListHeapState<K, N, List<V>>
	 ...
	 */
	private RemoteOperatorListState(
		...
		RemoteHeapSerializedCompositeKeyBuilder<K> sharedKeyNamespaceSerializer) {
		super(
			keySerializer,
			valueSerializer,
			namespaceSerializer,
		...
		try {
			...
			for (N source : sources) {
				if (source != null) {
					...
					byte[] valueBytes = syncRemClient.get(sourceKey);

					if (valueBytes != null) {
						syncRemClient.del(sourceKey);
						syncRemClient.rpush(targetKey, valueBytes);
					}
				}
			}
		} catch (Exception e) {
	...
	static <E, K, N, SV, S extends State, IS extends S> IS create(
		...
		TypeSerializer<K> keySerializer,
		HeapOperatorStateBackend backend) {
		...
		RemoteHeapSerializedCompositeKeyBuilder<K> sharedREMKeyBuilder = new RemoteHeapSerializedCompositeKeyBuilder<>(
			StateSerializerProvider.fromNewRegisteredSerializer(keySerializer).currentSchemaSerializer(),
			keyGroupPrefixBytes,
			32);
		return (IS) new RemoteOperatorListState<>(
			keySerializer,
			(TypeSerializer<List<E>>) metaInfo.getStateSerializer(),
			metaInfo.getNamespaceSerializer(),
			kvState,
			(List<E>) stateDesc.getDefaultValue(),
			backend.syncRemClient,sharedREMKeyBuilder);
	}
}
